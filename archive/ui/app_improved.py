"""
Improved MCP RAG Chatbot - Enhanced Streamlit UI
개선된 데이터 흐름을 지원하는 Streamlit UI
- MCP Tool 호출 실시간 표시
- 다단계 검색 진행 상황 추적
- 의도 분석 결과 표시
- 3000 토큰 제한 응답 지원
...
from src.agents.react_agent_improved import improved_react_agent
from src.utils.session import SessionManager
from config.settings import settings

# 페이지 설정
st.set_page_config(
    page_title="Enhanced MCP RAG Chatbot",
    page_icon="🤖",
    layout="wide",
    ...
    with progress_container:
        ...
        if tool_calls:
            ...
            for tool_call_key in tool_calls[-5:]:  # 최근 5개만 표시
                ...
                stage = call_info.get("stage", "unknown")
...
def display_search_quality_metrics(search_results: Dict[str, Any]):
    ...
    with st.expander("📊 검색 품질 메트릭", expanded=False):
        ...
        with col1:
            st.metric("총 Citation 수", quality_metrics.get("total_citations", 0))
        
        with col2:
            st.metric("고신뢰도 Citation", quality_metrics.get("high_confidence_citations", 0))
        
        with col3:
            st.metric("고유 문서 수", quality_metrics.get("unique_documents", 0))
...
def main():
    # 헤더
    st.title("🤖 Enhanced MCP RAG Chatbot")
    st.markdown("개선된 ReAct 기반 AI 어시스턴트 - 의도 분석 및 다단계 검색 지원")
    ...
    with st.sidebar:
        ...
        **Temperature**: 0 (정확성 우선)
        ...
        # 실행 통계
        if st.session_state.improved_react_agent:
            if st.button("📊 실행 통계 보기"):
                stats = st.session_state.improved_react_agent.get_enhanced_execution_stats()
                
                st.markdown("**최근 실행 통계:**")
                st.json({
                    "총 쿼리 수": stats.get("total_queries", 0),
                    "성공률": f"{stats.get('success_rate', 0)}%",
                    "평균 처리 시간": f"{stats.get('average_processing_time', 0)}초",
                    "평균 Citation 수": stats.get("average_citation_count", 0),
                    "다단계 검색 사용률": f"{stats.get('feature_usage', {}).get('multi_stage_search_rate', 0)}%"
                })
        ...
        with st.chat_message(message["role"]):
            if message["role"] == "user":
                ...
                if "citations" in message and message["citations"]:
                    ...
                    message_id = f"enhanced_msg_{idx}_{hash(str(message.get('timestamp', '')))}"
    ...
    if user_input:
        ...
        with st.chat_message("assistant"):
            ...
            if result:
                ...
                if citations:
                    from ui.app import display_citation_expandable_with_id
                    new_response_id = f"enhanced_new_{int(time.time() * 1000)}"
